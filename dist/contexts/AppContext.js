...
export const AppProvider = ({ appId, children }) => {
    ...
    const settings = useMemo(() => Object.fromEntries(data?.app_by_pk?.app_settings.map(v => [v.key, v.value]) || []), [data?.app_by_pk?.app_settings]);
    const secrets = useMemo(() => Object.fromEntries(data?.app_by_pk?.app_secrets.map(v => [v.key, v.value]) || []), [data?.app_by_pk?.app_secrets]);
    const app = useMemo(() => data?.app_by_pk
        ? {
            loading,
            error,
            refetch,
            ...
            currencies: data?.currency.reduce((accumulator, currency) => {
        ...
        : defaultAppContextProps, [data?.app_by_pk, data?.currency, error, loading, refetch, secrets, settings]);
    useEffect(() => {
        if (!authToken) {
            refreshToken?.();
        }
